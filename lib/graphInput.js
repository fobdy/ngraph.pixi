...
var debug = require('debug')('poincare:zoom');
/**
...
module.exports = function (graphics) {
  ...
  var sel = d3.select(graphics.domContainer);
  zoom(sel);

  zoom.on('zoom', function zm() {
    // debug('ZOOM', d3.event);
    graphGraphics.scale.x = d3.event.scale;
    graphGraphics.scale.y = d3.event.scale;

    graphGraphics.position.x = d3.event.translate[0];
    graphGraphics.position.y = d3.event.translate[1];
  });
